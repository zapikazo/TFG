...
char* getfield(FILE* file, char* taken){
	...
	ch = NULL;
	free(ch);
    taken = (char*)realloc(taken, elems);
	return taken;
}
...
vectorInt32Struct flipped_bits(int32_t word, int32_t pattern, int wordWidth){
    ...
    result.data = (int32_t*)calloc(wordWidth, sizeof(int32_t));
    ...
    if (nFound > 0) {
        result.data = (int32_t*)realloc(result.data, nFound*sizeof(int32_t));
    }
...
matrixUint322DStruct transposed_matrix(matrixUint322DStruct matrix){
    ...
    transposedMatrix.data = (uint32_t**)calloc(matrix.rows, sizeof(uint32_t*));
	for (int i = 0; i < transposedMatrix.rows; i++)
        transposedMatrix.data[i] = (uint32_t*)calloc(transposedMatrix.cols, sizeof(uint32_t));
...
matrixUint322DStruct create_DVmatrix(uint32_t* addresses, int elems, char* op, uint32_t* RWcycles, int nbits4blocks, long int ln){
    ...
    DVmatrix.rows = elems;
    DVmatrix.cols = elems;
    DVmatrix.data = (uint32_t**)calloc(DVmatrix.rows, sizeof(uint32_t *));
    for (i = 0; i < DVmatrix.rows; i++){
        DVmatrix.data[i] = (uint32_t*)calloc(DVmatrix.cols, sizeof(uint32_t));
    }
    ...
    DVmatrixOP.data = (uint32_t**)calloc(DVmatrixOP.rows, sizeof(uint32_t *));
    for (i = 0; i < DVmatrixOP.rows; i++){
        DVmatrixOP.data[i] = (uint32_t*)calloc(DVmatrixOP.cols, sizeof(uint32_t));
    }
    for (i = 0; i < DVmatrixOP.rows; i++) {
    ...
    coincidence.data = (uint32_t**)calloc(coincidence.rows, sizeof(uint32_t *));
    for (i = 0; i < elems; i++){
        coincidence.data[i] = (uint32_t*)calloc(coincidence.cols, sizeof(uint32_t));
    }
    ...
    int32_t* addressblocks = (int32_t*)calloc(elems, sizeof(int32_t));
	for (i = 0; i < coincidence.rows; i++) {
...
vectorUint32Struct create_DVvector(matrixUint322DStruct* DVmatrix){
    ...
    DVvector.data =(uint32_t*) calloc(ndv, sizeof(uint32_t));
	...
	* column and going down. */
    ...
    DVvector.data = (uint32_t*)realloc(DVvector.data, index*sizeof(uint32_t));
...
vectorInt32Struct countsInt(matrixInt322DStruct* matrix, int col, int maxValue){
    ...
    result.data = (int32_t*)calloc(result.length, sizeof(int32_t));
...
vectorInt32Struct create_histogram(vectorUint32Struct* vector, long int ln){
    ...
    histogram.length = ln;
    histogram.data = (int32_t*)calloc(ln, sizeof(int32_t));
	int k = 0, sum = 0;
...
matrixInt322DStruct vcat(matrixInt322DStruct* matrixA, matrixInt322DStruct* matrixB){
    ...
    matrixC.data = (int32_t**)calloc(matrixC.rows, sizeof(int32_t *));
    for (int i = 0; i < matrixC.rows; i++){
        matrixC.data[i] = (int32_t*)calloc(matrixA->cols, sizeof(int32_t));
    }
...
int32_t excessiveRepetitions(matrixInt322DStruct* repInHistVector, int repInHistVectorCol, long int LN, char* operation, double threshold){
    ...
    occurrenceIndex.data = (int32_t*)calloc(occurrenceIndex.length, sizeof(int32_t));
...
matrixInt322DStruct find_anomalies_histogram(matrixUint322DStruct *histogram, int histogramCol, matrixInt322DStruct *occurrences, int occurrencesCol, int32_t nthreshold, int* n_anomalous_values){ //n_anomalous_values por referencia porque necesitamos el valor
	...
	*/
    ...
    anomalies.cols = 2;
    anomalies.data = (int32_t**)calloc(anomalies.rows, sizeof(int32_t*));
	for (i = 0; i < anomalies.rows; i++) {
        anomalies.data[i] = (int32_t*)calloc(anomalies.cols, sizeof(int32_t));
	}
...
matrixInt322DStruct extract_some_critical_values(matrixInt322DStruct *anomalous_repetitions, int* n_anomalous_repetitions, int n){
	...
	if (*n_anomalous_repetitions <= n) {
        ...
        extracted_values.data = (int32_t**)calloc(extracted_values.rows, sizeof(int32_t*));
		for (k = 0; k < extracted_values.rows; k++) {
            extracted_values.data[k] = (int32_t*)calloc(extracted_values.cols, sizeof(int32_t));
		}
		for (k = 0; k < extracted_values.rows; k++) {
...
matrixBool2DStruct marking_addresses(matrixInt322DStruct* davmatrix, matrixInt322DStruct* critical_values){
    ...
    result.data = (bool**)calloc(result.rows, sizeof(bool*));
	for (i = 0; i < result.rows; i++) {
        result.data[i] = (bool*)calloc(result.cols, sizeof(bool));
	}
...
vectorInt32Struct unionAgrupate_mcus(matrixInt322DStruct* thesummary, int32_t addressRowMin, int32_t addressRowMax){
    ...
    thesummarytemp.data = (int32_t*)calloc(thesummarytemp.length, sizeof(int32_t));
...
matrixInt322DStruct agrupate_mcus(matrixBool2DStruct* davmatrix){
    ...
    nonzerovectorelements.data = (int32_t*)calloc(davmatrix->rows*davmatrix->cols, sizeof(int32_t));
    ...
    nonzerovectorelements.data = (int32_t*)realloc(nonzerovectorelements.data, nonzerovectorelements.length * sizeof(int32_t));
    ...
    relatedpairs.cols = 2;
    relatedpairs.data = (int32_t**)calloc(relatedpairs.rows, sizeof(int32_t*));
    for (i = 0; i < relatedpairs.rows; i++) {
        relatedpairs.data[i] = (int32_t*)calloc(relatedpairs.cols, sizeof(int32_t));
    }
    ...
    thesummary.data = (int32_t**)calloc(count, sizeof(int32_t*));
    if (count < NMaxAddressesInEvent) { /* If the minimun is newCount */
        thesummary.cols = count;
        for (i = 0; i < count; i++) {
            thesummary.data[i] = (int32_t*)calloc(thesummary.cols, sizeof(int32_t));
        }
    ...
    result.data = (int32_t**)calloc(result.rows, sizeof(int32_t*));
    for (i = 0; i < result.rows; i++) {
        result.data[i] = (int32_t*)calloc(result.cols, sizeof(int32_t));
    }
...
matrixInt323DStruct cutZerosFromMCUsummary(matrixInt323DStruct* MCUSummary){
    ...
    dimensionsMCU.data = (int32_t**)calloc(dimensionsMCU.rows, sizeof(int32_t*));
	for (i = 0; i < dimensionsMCU.rows; i++) {
        dimensionsMCU.data[i] = (int32_t*)calloc(dimensionsMCU.cols, sizeof(int32_t));
	}
	for (i = 0; i < dimensionsMCU.cols; i++) {
        ...
        tmpMatrix.data = (int32_t**)calloc(tmpMatrix.rows, sizeof(int32_t*));
        for (j = 0; j < tmpMatrix.rows; j++) {
            tmpMatrix.data[j] = (int32_t*)calloc(tmpMatrix.cols, sizeof(int32_t));
        }
        for (j = 0; j < tmpMatrix.rows; j++) {
    ...
    simpleMCUSummary.data = (int32_t***)calloc(simpleMCUSummary.rows, sizeof(int32_t**));
	for (i = 0; i < simpleMCUSummary.rows; i++) {
        simpleMCUSummary.data[i] = (int32_t**)calloc(simpleMCUSummary.cols, sizeof(int32_t*));
		for (j = 0; j < simpleMCUSummary.cols; j++) {
            simpleMCUSummary.data[i][j] = (int32_t*)calloc(simpleMCUSummary.dims, sizeof(int32_t));
		}
	}
...
vectorInt32Struct setdiffTraceRule(vectorInt32Struct *vector, matrixInt322DStruct *matrix){
    ...
    newVector.data = (int32_t*)calloc(newVector.length, sizeof(int32_t));
    ...
    newVector.data = (int32_t*)realloc(newVector.data, cont * sizeof(int32_t));
...
vectorInt32Struct unionVec(vectorInt32Struct *v1, vectorInt32Struct *v2, vectorInt32Struct *v3){
    ...
    if (v3 == NULL) { // Two vectors union
        ...
        unionVec.data = (int32_t*)calloc(unionVec.length, sizeof(int32_t));
        ...
        unionVec.data = (int32_t*)realloc(unionVec.data, cont * sizeof(int32_t));
...
vectorInt32Struct traceRule(matrixInt322DStruct *xorDVtotalrepetitions, matrixUint322DStruct *totalDVhistogram, matrixInt322DStruct *anomalXORvalues, long int LN){
    ...
    candidatesT1.data = (int32_t*)calloc(candidatesT1.length, sizeof(int32_t));
    ...
    selectedT1.data = (int32_t*)calloc(selectedT1.length, sizeof(int32_t)); //Luego se redimensiona al número de elemntos real
    int sT1Lenght = 0;
    ...
    selectedT1.data = (int32_t*)realloc(selectedT1.data, selectedT1.length*sizeof(int32_t));
    ...
    candidatesT2.data = (int32_t*)calloc(candidatesT2.length, sizeof(int32_t));
    index = 0;
    for (i = 0; i < (nBits-1); i++) {
        for (j = i+1; j < nBits; j++) {
    ...
    selectedT2.data = (int32_t*)calloc(candidatesT2.length, sizeof(int32_t)); //Luego se redimensiona al número de elemntos real
    int sT2Lenght = 0;
    ...
    selectedT2.data = (int32_t*)realloc(selectedT2.data, sT2Lenght*sizeof(int32_t));
    ...
    candidatesT3.data = (int32_t*)calloc(candidatesT3.length, sizeof(int32_t));
    index = 0;
    for (i = 0; i < (nBits-2); i++) {
    ...
    selectedT3.data = (int32_t*)calloc(candidatesT3.length, sizeof(int32_t)); //Luego se redimensiona al número de elemntos real
    int sT3Lenght = 0;
    ...
    selectedT3.data = (int32_t*)realloc(selectedT3.data, selectedT3.length*sizeof(int32_t));
...
matrixInt323DStruct propose_MCUs(char* op, matrixUint323DStruct *XORDVmatrix3D, matrixUint323DStruct *POSDVmatrix3D, matrixInt322DStruct *anomalXOR, matrixInt322DStruct *anomalPOS, vectorIntStruct* nAddressesInRound){
	...
	if (strcmp(op, "xor") == 0){ /* OP == XOR */
        ...
        XOR_summary.data = (int32_t***)calloc(XOR_summary.rows, sizeof(int32_t**));
		for (i = 0; i < XOR_summary.rows; i++) {
			XOR_summary.data[i] = (int32_t**)calloc(XOR_summary.cols, sizeof(int32_t*));
			for (j = 0; j < XOR_summary.cols; j++) {
				XOR_summary.data[i][j] = (int32_t*)calloc(XOR_summary.dims, sizeof(int32_t));
			}
		}
		for (ktest = 0; ktest < XOR_summary.dims; ktest++){
            ...
            xordvmatrix.data = (uint32_t**)calloc(xordvmatrix.rows, sizeof(int32_t*));
			for (i = 0; i < xordvmatrix.rows; i++) {
                xordvmatrix.data[i] = (uint32_t*)calloc(xordvmatrix.cols, sizeof(int32_t));
			}
            copyOfMatrix3to2((matrixInt323DStruct*)XORDVmatrix3D, &xordvmatrix, ktest);
            
            matrixBool2DStruct XORmarked_pairs = marking_addresses((matrixInt322DStruct*)&xordvmatrix, anomalXOR);
        ...
        POS_summary.data = (int32_t***)calloc(POS_summary.rows, sizeof(int32_t**));
		for (i = 0; i < POS_summary.rows; i++) {
			POS_summary.data[i] = (int32_t**)calloc(POS_summary.cols, sizeof(int32_t*));
			for (j = 0; j < POS_summary.cols; j++) {
				POS_summary.data[i][j] = (int32_t*)calloc(POS_summary.dims, sizeof(int32_t));
			}
		}
		...
		for (ktest = 0; ktest < XORDVmatrix3D->dims; ktest++){
            ...
            posdvmatrix.data = (int32_t**)calloc(posdvmatrix.rows, sizeof(int32_t*));
			for (i = 0; i < posdvmatrix.rows; i++) {
				posdvmatrix.data[i] = (int32_t*)calloc(posdvmatrix.cols, sizeof(int32_t));
			}
            copyOfMatrix3to2((matrixInt323DStruct*)POSDVmatrix3D, (matrixUint322DStruct*)&posdvmatrix, ktest);
        ...
        CMB_summary.data = (int32_t***)calloc(CMB_summary.rows, sizeof(int32_t**));
		for (i = 0; i < CMB_summary.rows; i++) {
			CMB_summary.data[i] = (int32_t**)calloc(CMB_summary.cols, sizeof(int32_t*));
			for (j = 0; j < CMB_summary.cols; j++) {
				CMB_summary.data[i][j] = (int32_t*)calloc(CMB_summary.dims, sizeof(int32_t));
			}
		}
		...
		for (ktest = 0; ktest < XORDVmatrix3D->dims; ktest++){
            ...
            xordvmatrix.rows = nAddressesInRound->data[ktest];
            xordvmatrix.cols = nAddressesInRound->data[ktest];
            xordvmatrix.data = (int32_t**)calloc(xordvmatrix.rows, sizeof(int32_t*));
			for (i = 0; i < xordvmatrix.rows; i++) {
				xordvmatrix.data[i] = (int32_t*)calloc(xordvmatrix.cols, sizeof(int32_t));
			}
            copyOfMatrix3to2((matrixInt323DStruct*)XORDVmatrix3D, (matrixUint322DStruct*)&xordvmatrix, ktest);
            ...
            XOR_POS_marked_pairs.data = (bool**)calloc(nAddressesInRound->data[ktest], sizeof(bool*));
			for (i = 0; i < nAddressesInRound->data[ktest]; i++) {
				XOR_POS_marked_pairs.data[i] = (bool*)calloc(nAddressesInRound->data[ktest], sizeof(bool));
			}
...
matrixInt322DStruct condensate_summary(matrixInt323DStruct* summary3D, vectorIntStruct* nAddressesInRound){
    ...
    condensateSummary.data = (int32_t**)calloc(condensateSummary.rows, sizeof(int32_t*));
	for (i = 0; i < condensateSummary.rows; i++) {
		condensateSummary.data[i] = (int32_t*)calloc(condensateSummary.cols + 1, sizeof(int32_t));
	...
	for (i = 1; i < summary3D->dims + 1; i++) {
        ...
        summary2D.data = (int32_t**)calloc(summary2D.rows, sizeof(int32_t*));
        for (k = 0; k < summary2D.rows; k++) {
            summary2D.data[k] = (int32_t*)calloc(summary2D.cols, sizeof(int32_t));
        }
        copyOfMatrix3to2(summary3D, (matrixUint322DStruct*)&summary2D, (i-1));
...
matrixInt322DStruct locate_mbus(matrixInt322DStruct* content, int** pattern, int nRoundsInPattern, int datawidth){
    ...
    summary.cols = 4;
    summary.data = (int32_t**)calloc(summary.rows, sizeof(uint32_t*));
	for (i = 0; i < summary.rows; i++) {
        summary.data[i] = (int32_t*)calloc(summary.cols, sizeof(uint32_t));
	}
...
matrixInt322DStruct extractAnomalDVSelfConsistency(char* op, matrixInt322DStruct* opDVtotalrepetitions, matrixUint322DStruct* totalDVhistogram, vectorIntStruct* nAddressesInRound, matrixInt322DStruct* opdvhistogram, matrixInt323DStruct* opdvmatrixbackup, int* XORANOMALS){
    ...
    int* n_anomalous_values = (int*)malloc(sizeof(int));
    ...
    if (strcmp(op, "xor") == 0) {
        ...
        *XORANOMALS = *n_anomalous_values;
		...
		while (oPSelfConsistence) {
            ...
            opPartRepsSelfCons.data = (int32_t**)calloc(opPartRepsSelfCons.rows, sizeof(int32_t*));
			for (i = 0; i < opPartRepsSelfCons.rows; i++) {
				opPartRepsSelfCons.data[i] = (int32_t*)calloc(opPartRepsSelfCons.cols, sizeof(int32_t));
			}
			for (i = 0; i < opPartRepsSelfCons.rows; i++) {
			...
			for (test = 0; test < nAddressesInRound->length; test++) {
                ...
                opdvmatrix.data = (uint32_t**)calloc(opdvmatrix.rows, sizeof(int32_t*));
				for (i = 0; i < opdvmatrix.rows; i++) {
                    opdvmatrix.data[i] = (uint32_t*)calloc(opdvmatrix.cols, sizeof(int32_t));
				}
                copyOfMatrix3to2(opdvmatrixbackup, &opdvmatrix, test);
                matrixBool2DStruct opMarkedPairs;
                opMarkedPairs = marking_addresses((matrixInt322DStruct*)&opdvmatrix, &oPExtracted_values);
        ...
        while (oPSelfConsistence) {
			...
			for (test = 0; test < nAddressesInRound->length; test++) {
                ...
                copyOfMatrix3to2(opdvmatrixbackup, &opdvmatrix, test);
                matrixBool2DStruct opMarkedPairs = marking_addresses((matrixInt322DStruct*)&opdvmatrix, &oPExtracted_values);
...
matrixInt323DStruct index2address(matrixInt323DStruct* indexMatrix, matrixInt322DStruct* addressMatrix){
	...
	for (i = 0; i < result.rows; i++) {
		...
		for (j = 0; j < result.cols; j++) {
			result.data[i][j] = (int32_t*)calloc(result.dims, sizeof(int32_t));
		}
	}
...
vectorInt32Struct criticalXORValuesFromClusters(matrixInt323DStruct* PrelMCUSummary, matrixInt322DStruct* AddressMatrix, matrixInt322DStruct* XORextracted_values, matrixInt322DStruct* xorDVtotalrepetitions, matrixInt322DStruct* DVHistogram){
    ...
    newCandidates.data = (int32_t*)calloc(PrelMCUSummary->rows * PrelMCUSummary->cols * PrelMCUSummary->dims, sizeof(int32_t));
				...
				if (index1 == 0){
					...
					for (kAddress2 = 0; kAddress2 < kAddress1; kAddress2++){
						...
						if (findEqualMatrix(XORextracted_values, candidate) == 0){
                            ...
                            vCandidate.length = 1;
                            vCandidate.data = (int32_t*)calloc(1, sizeof(int32_t));
    ...
    newCandidates.data = (int32_t*)realloc(newCandidates.data, newCandidates.length * sizeof(int32_t));
    ...
    purgedCandidatesXOR.data = (int32_t*)calloc(purgedCandidatesXOR.length, sizeof(int32_t));
    ...
    purgedCandidatesXOR.data = (int32_t*)realloc(purgedCandidatesXOR.data, purgedCandidatesXOR.length*sizeof(int32_t));
...
void extractAnomalDVfromClusters(matrixInt322DStruct* content, matrixInt322DStruct* XORextracted_values, matrixInt322DStruct* POSextracted_values, matrixInt322DStruct* xorDVtotalrepetitions, matrixInt322DStruct* posDVtotalrepetitions, matrixUint322DStruct* totalDVhistogram, matrixInt323DStruct* xordvmatrixbackup, matrixInt323DStruct* posdvmatrixbackup, vectorIntStruct* nAddressesInRound, long int LN, vectorInt32Struct* discoveredXORDVs, vectorInt32Struct* discoveredPOSDVs, matrixInt322DStruct* tempXORDVvalues, matrixInt322DStruct* tempPOSDVvalues){
    ...
    tempXORDVvalues->data = (int32_t**)calloc(tempXORDVvalues->rows, sizeof(int32_t*));
    for (i = 0; i < tempXORDVvalues->rows; i++) {
        tempXORDVvalues->data[i] = (int32_t*)calloc(tempXORDVvalues->cols, sizeof(int32_t));
    }

    tempPOSDVvalues->rows = POSextracted_values->rows;
    tempPOSDVvalues->cols = POSextracted_values->cols;
    tempPOSDVvalues->data = (int32_t**)calloc(tempPOSDVvalues->rows, sizeof(int32_t*));
    for (i = 0; i < tempPOSDVvalues->rows; i++) {
        tempPOSDVvalues->data[i] = (int32_t*)calloc(tempPOSDVvalues->cols, sizeof(int32_t));
    }
    ...
    while (foundNewDVValues) {
        ...
        tempXOR_summary = propose_MCUs("xor", (matrixUint323DStruct*)xordvmatrixbackup, (matrixUint323DStruct*)posdvmatrixbackup, tempXORDVvalues, tempPOSDVvalues, nAddressesInRound);
        tempPOS_summary = propose_MCUs("pos", (matrixUint323DStruct*)xordvmatrixbackup, (matrixUint323DStruct*)posdvmatrixbackup, tempXORDVvalues, tempPOSDVvalues, nAddressesInRound);
        tempCMB_summary = propose_MCUs("cmb", (matrixUint323DStruct*)xordvmatrixbackup, (matrixUint323DStruct*)posdvmatrixbackup, tempXORDVvalues, tempPOSDVvalues, nAddressesInRound);
        ...
        proposedXORDV = criticalXORValuesFromClusters(&tempCMB_summary, content, tempXORDVvalues, xorDVtotalrepetitions, (matrixInt322DStruct*)totalDVhistogram);
        ...
        if (nProposedXORDV != 0) {
            ...
            *discoveredXORDVs = unionVec(&proposedXORDV, discoveredXORDVs, NULL);
            ...
            auxXORDV.data = (int32_t**)calloc(auxXORDV.rows, sizeof(int32_t*));
            for (i = 0; i < auxXORDV.rows; i++) {
                auxXORDV.data[i] = (int32_t*)calloc(auxXORDV.cols, sizeof(int32_t));
            }
        ...
        if (nProposedPOSDV != 0) {
            ...
            *discoveredPOSDVs = unionVec(&proposedPOSDV, discoveredPOSDVs, NULL);
            ...
            auxPOSDV.data = (int32_t**)calloc(auxPOSDV.rows, sizeof(int32_t*));
            for (i = 0; i < auxPOSDV.rows; i++) {
                auxPOSDV.data[i] = (int32_t*)calloc(auxPOSDV.cols, sizeof(int32_t));
            }
            for (i = 0; i < auxPOSDV.rows; i++) {
...
matrixInt322DStruct criticalXORvaluesFromXORingRule(matrixInt322DStruct* XORextracted_values, matrixInt322DStruct* XORDVtotalrepetitions, matrixUint322DStruct* totalDVhistogram, vectorInt32Struct* candidates){
    ...
    prelCandidates.data = (int32_t*)calloc(1, sizeof(int32_t));
    prelCandidates.length = 0;

    vectorInt32Struct oldXORValues;
    oldXORValues.length = XORextracted_values->rows;
    oldXORValues.data = (int32_t*)calloc(oldXORValues.length, sizeof(int32_t));
    ...
    candidatesCase1.data = (int32_t*)calloc(prelCandidates.length, sizeof(int32_t)); //Luego se redimensiona al número de elemntos real
    ...
    candidatesCase1.data = (int32_t*)realloc(candidatesCase1.data, candidatesCase1.length*sizeof(int32_t));
    ...
    DVElements.data = (int32_t*)calloc(DVElements.length, sizeof(int32_t));
		int index = 0;
    ...
    selectedPrelCandidates.data = (int32_t*)calloc(1, sizeof(int32_t));
    printf(" Searching... ");
    int kdv, kxor;
		...
		for (kxor = 0; kxor < oldXORValues.length; kxor++){
			...
			//presencePrelCandidates[xored + 1] = true;
               if ((findEqualVector(&oldXORValues, xored) != 0) && (findEqualVector(&oldXORValues, kdv) == 0)) {
                   ...
                   vKdv.length = 1;
                   vKdv.data = (int32_t*)calloc(1, sizeof(int32_t));
                   vKdv.data[0] = kdv;
    ...
    candidatesCase2.data = (int32_t*)calloc(selectedPrelCandidates.length, sizeof(int32_t)); //Luego se redimensiona al número de elemntos real
    ...
    candidatesCase2.data = (int32_t*)realloc(candidatesCase2.data, candidatesCase2.length*sizeof(int32_t));
    ...
    *candidates = unionVec(&candidatesCase1, &candidatesCase2, NULL);
    ...
    newXORDVvalues.data = (int32_t**)calloc(XORextracted_values->rows, sizeof(int32_t*));
    for (i = 0; i < newXORDVvalues.rows; i++) {
        newXORDVvalues.data[i] = (int32_t*)calloc(2, sizeof(int32_t));
    }
    index = 0;
    for (i = 0; i < XORextracted_values->rows; i++) {
        for (j = 0; j < 2; j++) {
